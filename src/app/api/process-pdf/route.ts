...
export async function POST(request: NextRequest) {
  try {
    ...
    const inputRouterResult = await callAgent(
      ...
      `Prepare study materials from this PDF: ${blob.url}`,
      apiKey,
      [blob.url]
    );
    ...
    // Parse the response to separate notes and questions
    let notes = '';
    let questions = '';
    ...
    } else if (lowerResponse.includes('## notes') && lowerResponse.includes('## questions')) {
      const notesIndex = lowerResponse.indexOf('## notes');
      const questionsIndex = lowerResponse.indexOf('## questions');
      
      if (notesIndex < questionsIndex) {
        notes = finalResponse.substring(notesIndex + 8, questionsIndex).trim();
        questions = finalResponse.substring(questionsIndex + 12).trim();
      } else {
        questions = finalResponse.substring(questionsIndex + 12, notesIndex).trim();
        notes = finalResponse.substring(notesIndex + 8).trim();
      }
    } else {
      // If can't parse, use notes from Smart Note Generator and questions from Practice Question Generator
